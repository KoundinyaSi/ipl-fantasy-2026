...
export async function POST(request: Request) {
  ...
  if (!actual_first || !actual_second) {
    return NextResponse.json(
      { error: "actual_first and actual_second required" },
      { status: 400 },
    );
  }
  ...
  if (config?.results_processed) {
    return NextResponse.json(
      { error: "Results already processed" },
      { status: 400 },
    );
  }
  ...
  for (const pick of picks ?? []) {
    ...
    // +10 for first, +5 for second
    let points = 0;
    if (firstCorrect) points += 10;
    else if (secondCorrect) points += 5;
