...
export default function ProfileMenu({ user }: ProfileMenuProps) {
  ...
  async function handleSync() {
    ...
    try {
      ...
      alert(
        res.ok
          ? `✅ Synced ${data.synced} matches, resolved ${data.resultsProcessed} results.`
  ...
  const accuracy =
    user.total_predictions && user.total_predictions > 0
      ? Math.round(
          ((user.correct_predictions || 0) / user.total_predictions) * 100,
        )
      : null;
            ...
            <div className="px-5 py-3 border-b border-brand-border">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-white font-medium">
                  {user.login_streak > 0
                    ? `${user.login_streak} login${user.login_streak !== 1 ? "s" : ""}`
                    : "—"}
                </span>
              </div>
              <div className="flex items-center justify-between text-sm mt-2">
                ...
                <span
                  ...
                  style={{
                    color: user.voting_streak > 0 ? "#FF6B2B" : "#6B6B8A",
                  }}
                >
...
function Playing11AdminModalWrapper({ onClose }: { onClose: () => void }) {
  const [matches, setMatches] = useState<
    Array<{ id: string; team1: string; team2: string; match_date: string }>
  >([]);
  const supabase = createClient();
    ...
    return (
      ...
      >
        <div
          ...
          style={{ borderColor: "#FFD70033", borderTopColor: "#FFD700" }}
        />
      </div>
    );
  }
...
function StatBadge({
  ...
  return (
    ...
    >
      ...
      <div className="font-display text-white font-semibold text-sm">
        {value}
      </div>
