...
function getPlayoffStage(matchName: string): string | null {
  if (/qualifier 1/i.test(matchName)) return "Qualifier 1";
  if (/qualifier 2/i.test(matchName)) return "Qualifier 2";
  if (/eliminator/i.test(matchName)) return "Eliminator";
  if (/\bfinal\b/i.test(matchName)) return "Final";
  return null;
}
...
function getStageStyles(stage: string | null) {
  switch (stage) {
    case "Final":
      return {
        glow: "0 0 0 1px rgba(255,215,0,0.18), 0 8px 32px rgba(255,215,0,0.12)",
        accent: "linear-gradient(90deg, #FFD700, #FFB800)",
        badgeBg: "#FFD70022",
        badgeBorder: "#FFD70044",
        badgeText: "#FFD700",
        label: "🏆 FINAL",
      };
    ...
    case "Qualifier 1":
      return {
        glow: "0 0 0 1px rgba(255,107,43,0.16), 0 8px 28px rgba(255,107,43,0.10)",
        accent: "linear-gradient(90deg, #FF6B2B, #FF8C5A)",
        badgeBg: "#FF6B2B22",
        badgeBorder: "#FF6B2B44",
        badgeText: "#FF8C5A",
        label: "🔥 QUALIFIER 1",
      };
    ...
    case "Qualifier 2":
      return {
        glow: "0 0 0 1px rgba(168,85,247,0.16), 0 8px 28px rgba(168,85,247,0.10)",
    ...
    case "Eliminator":
      return {
        glow: "0 0 0 1px rgba(239,68,68,0.16), 0 8px 28px rgba(239,68,68,0.10)",
...
function getCompactStageLabel(stage: string) {
  switch (stage) {
    case "Qualifier 1":
      return "Q1";
    case "Qualifier 2":
...
export default function MatchCard({
  ...
  const hasTBDTeams =
    match.team1.toLowerCase() === "tbc" || match.team2.toLowerCase() === "tbc";

  const [voting, setVoting] = useState(false);
  const [localPrediction, setLocalPrediction] = useState(userPrediction);
  const [locked, setLocked] = useState(
    () => hasTBDTeams || isVotingLocked(match.match_date),
  );
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setLocked(hasTBDTeams || isVotingLocked(match.match_date));
    }, 30_000);
  ...
  const team1Voters = allPredictions.filter(
    (p) => p.predicted_team === match.team1,
  );
  const team2Voters = allPredictions.filter(
    (p) => p.predicted_team === match.team2,
  );
  const totalVotes = allPredictions.length;
  const team1Pct =
    totalVotes > 0 ? Math.round((team1Voters.length / totalVotes) * 100) : 50;
  const team2Pct = 100 - team1Pct;

  const playoffStage = getPlayoffStage(match.name);
  const playoffStyles = getStageStyles(playoffStage);

  return (
    <>
      <div
        className="glass rounded-2xl overflow-hidden transition-all duration-300 relative"
        style={{
          border: playoffStage
            ? "1px solid rgba(255,255,255,0.08)"
            : "1px solid rgba(255,255,255,0.05)",
          boxShadow: playoffStyles?.glow,
        }}
      >
        {/* Match header */}
        <div className="px-4 pt-4 pb-2 flex items-start justify-between gap-2">
          <div>
            <div className="text-xs text-brand-muted font-body">
              {formatMatchDate(match.match_date)}
            </div>
            {match.venue && (
              <div className="text-xs text-brand-muted/50 mt-0.5 truncate max-w-[200px]">
                📍 {match.venue}
              </div>
            )}
          </div>
            ...
            {match.match_started && !match.match_ended ? (
              ...
              <span
                ...
                style={{
                  background: "#6B6B8A22",
                  color: "#C4C4D4",
                  border: "1px solid #6B6B8A33",
                }}
              >
                {hasTBDTeams ? "⏳ Awaiting teams" : "🔒 Locked"}
              </span>
            ) : countdown ? (
              <span
        ...
        <div className="px-4 py-4 flex items-center justify-between gap-4">
          ...
          <div className="flex flex-col items-center gap-1 flex-1">
            {playoffStage && (
              <div className="flex items-center gap-1.5 mb-1">
                <div
                  className="h-px w-4"
                  style={{
                    background: `linear-gradient(to right, transparent, ${playoffStyles?.badgeText}66)`,
                  }}
                />

                <div
                  className="px-2 py-[3px] rounded-full backdrop-blur-sm"
                  style={{
                    background: `${playoffStyles?.badgeText}10`,
                    border: `1px solid ${playoffStyles?.badgeText}22`,
                  }}
                >
                  <div
                    className="text-[9px] font-bold uppercase leading-none"
                    style={{
                      color: playoffStyles?.badgeText,
                      letterSpacing: "0.12em",
                    }}
                  >
                    <span className="sm:hidden">
                      {getCompactStageLabel(playoffStage)}
                    </span>

                    <span className="hidden sm:inline">{playoffStage}</span>
                  </div>
                </div>
                ...
                <div
                  className="h-px w-4"
                  style={{
                    background: `linear-gradient(to left, transparent, ${playoffStyles?.badgeText}66)`,
                  }}
                />
              </div>
            )}
            ...
            {!localPrediction && locked && !hasTBDTeams && (
          ...
          >
            ...
            <div className="flex justify-between mt-1.5">
              ...
              <span className="text-xs text-brand-muted/50 flex items-center gap-1">
                ...
                <span className="text-[10px] text-brand-muted/30">
                  · tap to see
                </span>
              </span>
